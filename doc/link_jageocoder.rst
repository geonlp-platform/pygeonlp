...
2. ``jageocoder.init()`` を呼んで jageocoder を利用可能にします。
3. `pygeonlp.api パッケージ <pygeonlp.api.html>`_ を import します。
4. `api.init() <pygeonlp.api.html#pygeonlp.api.init>`_ を呼んで pygeonlp.api を利用可能にします。
5. `api.geoparse() <pygeonlp.api.html#pygeonlp.api.geoparse>`_ を jageocoder オプション付きで実行します。
